...
(() => {
  ...
  async function acknowledgePendingPushes() {
    try {
      ...
      M.cache.clear();
      document.querySelector('[data-m-push-overlay]')?.remove();
  ...
  async function renderNow() {
    ...
    const [data,judge,pending]=await Promise.all([api('/api/home',fallback),api('/api/xinchao/judge',{baselines:FALLBACK_BASE}),api('/api/behavior/pending',{available:false,count:0})]); const pipes=data.state?.pipes||{};
    ...
    stage().innerHTML=`<section class="m-now">
      ...
      ${pending.available&&!pending.acknowledged?`<aside class="m-push-overlay" data-m-push-overlay role="dialog" aria-modal="true" aria-label="新的推送"><div><span>有新的推送</span><strong>${num(pending.count)} 条内容在等你看见</strong><p>确认后，这一批不会再重复出现。</p><button class="m-primary" data-m-acknowledge type="button">我看到了</button></div></aside>`:''}
    </section>`;
