...
(() => {
  ...
  async function renderSettings(){const [judge,behavior]=await Promise.all([api('/api/xinchao/judge',{custom_rules:'',proxy_voice:'',darkflow_rules:'',relations:[]}),api('/api/behavior/settings',{push_title:'Clio'})]);stage().innerHTML=`<form id="mSettings" class="m-settings"><label>推送名称<input name="pushTitle" value="${safe(behavior.push_title||'Clio')}"/></label><label>机器判定规则<textarea name="rules" rows="5">${safe(judge.custom_rules||'')}</textarea></label><label>AI 表达口吻<textarea name="voice" rows="4">${safe(judge.proxy_voice||'')}</textarea></label><label>暗涌生成口吻与人物性格<textarea name="darkflow" rows="5">${safe(judge.darkflow_rules||'')}</textarea></label><button class="m-primary" type="submit">保存设置</button></form>`;document.querySelector('#mSettings')?.addEventListener('submit',async event=>{event.preventDefault();const fd=new FormData(event.currentTarget);try{await liveApi('/api/behavior/settings',{method:'PUT',body:JSON.stringify({push_title:fd.get('pushTitle')})});await liveApi('/api/xinchao/judge',{method:'PUT',body:JSON.stringify({custom_rules:fd.get('rules'),proxy_voice:fd.get('voice'),darkflow_rules:fd.get('darkflow'),baselines:judge.baselines||{},relations:judge.relations||[]})});M.cache.clear();liveNotice('设置已保存。')}catch(error){liveNotice(error.message,true)}});}
