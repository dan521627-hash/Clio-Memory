...
(() => {
  ...
  async function renderBehavior(){
    const [data,pending]=await Promise.all([api('/api/behavior/actions?limit=80',{sent_items:[],decisions:[],trajectory:{}}),api('/api/behavior/pending',{available:false,count:0})]);const sent=data.sent_items||[];const decisions=data.decisions||[];const trace=data.trajectory||{};const drivers=trace.emotion_drivers||[];
    const driverText=drivers.slice(0,6).map(item=>`${item.name} ${num(item.value).toFixed(2)}`).join(' · ')||'当前没有明显牵动';
    stage().innerHTML=`<section class="m-behavior"><div class="m-push-journey"><span>这一轮进行到哪里</span><h2>${safe(trace.phase_label||'等待新一轮')}</h2><p>${safe(trace.phase_detail||'')}</p><ol><li><i></i><div><small>本轮触发</small><strong>${safe(trace.trigger_summary||'本轮没有单独的文字写入')}</strong><time>${safe(time({created_at:trace.last_activity_at}))}</time></div></li><li><i></i><div><small>30分钟候静默</small><strong>${trace.phase==='active'?'正在重新计时':'已经完整结束'}</strong>${trace.silence_due_at?`<time>${safe(time({created_at:trace.silence_due_at}))}</time>`:''}</div></li><li><i></i><div><small>${num(trace.pipe_count)||48}项情绪参与判断</small><strong>${safe(driverText)}</strong></div></li><li><i></i><div><small>DeepSeek判断</small><strong>${safe(trace.phase_label||'等待判断')}</strong>${trace.next_review_at?`<time>下次复核 ${safe(time({created_at:trace.next_review_at}))}</time>`:''}</div></li></ol></div>${pending.available&&!pending.acknowledged?`<button class="m-primary m-acknowledge" data-m-acknowledge type="button">我看到了 · ${num(pending.count)} 条</button>`:''}<div class="m-section-head"><div><span>已经送达</span><h2>${sent.length} 条推送</h2></div></div><div class="m-sent-list">${sent.map(item=>`<article><time>${safe(time(item))}</time><span>${safe(item.status_label||'已推送')}</span><strong>${safe(item.display_content||text(item)||'已完成一次推送')}</strong>${item.trigger_summary?`<p>因为：${safe(item.trigger_summary)}</p>`:''}${item.display_reason?`<p>${safe(item.display_reason)}</p>`:''}</article>`).join('')||empty('最近没有已发送的推送')}</div><details class="m-decisions"><summary>为什么没发、等待或取消 <b>${decisions.length}</b></summary>${decisions.map(item=>`<article><time>${safe(time(item))}</time><strong>${safe(item.status_label||'未发送')}</strong>${item.trigger_summary?`<p>本轮写入：${safe(item.trigger_summary)}</p>`:''}<p>${safe(item.display_reason||'本次没有发送')}</p>${item.next_review_at?`<p>下次复核：${safe(time({created_at:item.next_review_at}))}</p>`:''}</article>`).join('')||empty('没有未发送记录')}</details></section>`;
    stage().querySelector('[data-m-acknowledge]')?.addEventListener('click',async()=>{try{const result=await liveApi('/api/behavior/acknowledge',{method:'POST',body:'{}'});M.cache.clear();liveNotice(result.message||'已经记下你看到了。');await navigate('behavior',false)}catch(error){liveNotice(error.message,true)}});
  }
