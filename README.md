...
<img src="docs/images/v2.0/mobile/01-mobile.png" alt="Anima 2.0 手机版总览" width="540">

## 2.0 多了什么

- **连续事件摘要**：把最近六封信整理成一段有前后关系的事件摘要；相同事情只说一次，但保留后来发生的变化，不再夹带“已完成、待确认”等系统状态词。
- **统一时间线**：记忆不再只是散落的条目，可以看见一件事如何开始、改变和延续。
- **写入后自动续写时间线**：识别到明确变化后直接形成新节点，不再停在“等待确认”。
- **人物关系记忆**：允许按人物建立主题，新的相关记忆可以自动形成关系线索。
- **主题可以自己建立**：电脑端和手机端都能新增“大主题 + 子目录”，不需要修改代码，也不需要请维护者从后台添加。
- **新记忆按主题归位**：写入后优先根据内容进入合适主题；自定义子目录名称也会参与判断。无法可靠判断时进入“待分类 / 需要整理”，不会拿标签冒充主题。
...
适合手机保存和转发的完整六张图，见 [2.0 手机图组](docs/images/v2.0/mobile/README.md)。
...
默认主题和演示数据均使用通用称呼。主题属于每个安装者自己的数据，可以在网页中自行创建；代码中不预置维护者或任何真实用户的名字。
