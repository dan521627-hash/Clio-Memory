...
事件与感受进入状态评估，影响激素、心念、共振和张力
        ↓
内容明确表示收尾后进入静默交接，再形成暗涌并按规则产生可选推送
        ↓
...
- [v1.4.2 手机首页推送确认说明](RELEASE-NOTES-v1.4.2.md)
- [v1.4.1 真静默、暗涌与推送说明](RELEASE-NOTES-v1.4.1.md)
...
不要把 `.env.example` 直接当作正式配置启动。服务会拒绝使用示例验真暗语，
管理页也不会接受示例密码；请使用 `start.sh` / `start.ps1` 自动生成，或自行填写随机私密值。

健康检查中的 `decay_engine` 在首次执行记忆读写前会显示
...
9. **未竟事项只用于可完成的行动。** 长期相处态度、关系承诺、道歉后的表态和人物行为准则不会自动创建待办。
